...
const BetaBanner: React.FC<BetaBannerProps> = ({ version = getAppVersion() }) => {
  ...
  return (
    <div
      ...
      // On mobile devices, fill the top safe area with the same background color so the
      // status bar area is not white. Use paddingTop instead of marginTop. For Android
      // devices, supply a fixed fallback of ~25px because env(safe-area-inset-top)
      // typically returns 0 on devices without a notch. On iOS, rely on the safe-area value.
      style={{
        paddingTop: mobile ? (isAndroid() ? '25px' : 'env(safe-area-inset-top)') : '0'
      }}
    >
